...
fn single_claim_succeeds() {
    new_test_ext().execute_with(|| {
        ...
        assert!(System::events().iter().any(|er| er.event
            == TestEvent::Faucet(Event::FaucetDripped(drip_amount, 1)
		)));
		assert!(System::events().iter().any(|er| er.event
            == TestEvent::Faucet(Event::FaucetDripped(drip_amount, 2)
		)));
    });
}
...
fn multiple_claims_fail_when_too_recent() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Faucet::claim_tokens(account.clone()),
			Error::<Test>::LastClaimTooRecent
		);
